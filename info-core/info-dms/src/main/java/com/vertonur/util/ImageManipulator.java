...
public class ImageManipulator {
	 ...
	 *            as PNG.
	 * 
	 *            This class isn't need cuz it only support JPG format public
	 *            static void saveCompressedImage(BufferedImage image, String
	 *            toFileName, int type) { try { if (type == IMAGE_PNG) { throw
	 *            new UnsupportedOperationException(
	 *            "PNG compression not implemented"); }
	 * 
	 *            Iterator iter = ImageIO.getImageWritersByFormatName("jpg");
	 *            ImageWriter writer; writer = (ImageWriter) iter.next();
	 * 
	 *            ImageOutputStream ios = ImageIO.createImageOutputStream(new
	 *            File(toFileName)); writer.setOutput(ios);
	 * 
	 ...
	 * 
	 *            ios.flush(); writer.dispose(); ios.close(); } catch
	 *            (IOException e) { //throw new ForumException(e); } }
	 */
	...
	public static BufferedImage createHeadlessBufferedImage(
		...
		if (type == ImageManipulator.IMAGE_PNG && hasAlpha(image)) {
		...
		for (int y = 0; y < height; y++) {
			for (int x = 0; x < width; x++) {
				bi.setRGB(
						x,
						y,
						image.getRGB(x * image.getWidth() / width,
	...
	public static BufferedImage createHeadlessSmoothBufferedImage(
		...
		if (type == ImageManipulator.IMAGE_PNG && hasAlpha(source)) {
			type = BufferedImage.TYPE_INT_ARGB;
		} else {
			type = BufferedImage.TYPE_INT_RGB;
		}
